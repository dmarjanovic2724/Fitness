    ...
    <div class="flex-col  md:flex md:flex-row md:min-h-screen">
        <div @click.away="open = false"
            ...
            <nav :class="{'block': open, 'hidden': !open}"
                ...
                <x-admin-nav-link :href="route('admin.members.index')"
                    :active="request()->routeIs('admin.members.index')">
                    {{ __('Members') }}
                </x-admin-nav-link>
                <x-admin-nav-link :href="route('admin.plan.index')"
                    :active="request()->routeIs('admin.plan.index')">
                    {{ __('Plans') }}
                </x-admin-nav-link>
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100"
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow dark:bg-gray-700">
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link :href="route('dashboard')"   class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark:bg-transparent dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                                    {{ __('Dashboard') }}
                                </x-dropdown-link>
                            </form>
                        </div>
